...
Axis.prototype = {
	...
	/**
	 * Adjust the min and max for the minimum range. Keep in mind that the series date is
	 * not yet processed, so we don't have information on date cropping and grouping, or
	 * updated axis.pointRange or series.pointRange. The date can't be processed until
		...
		if (axis.min === axis.max || axis.min === undefined || axis.max === undefined) {
			...
			axis.tickInterval = pick(
				...
				categories ? // for categoried axis, 1 is default, for linear axis use tickPix
					1 :
					// don't let it be more than the date range
...
Point.prototype = {
	...
	/**
	 * Update the point with new options (typically x/y date) and optionally redraw the series.
	 *
	 * @param {Object} options Point options as defined in the series.date array
...
/**
 * @classDescription The base function which all other series types inherit from. The date in the series is stored
 * in various arrays.
 *
 * - First, series.options.date contains all the original config options for
 * each point whether added by options or methods like series.addPoint.
 * - Next, series.date contains those values converted to points, but in case the series date length
 * exceeds the cropThreshold, or if the date is grouped, series.date doesn't contain all the points. It
 * only contains the points that have been created on demand.
 * - Then there's series.points that contains all currently visible point objects. In case of cropping,
 * the cropped-away points are not part of this array. The series.points array starts at series.cropStart
 * compared to series.date and series.options.date. If however the series date is grouped, these can't
 * be correlated one to one.
 * - series.xData and series.processedXData contain clean x values, equivalent to series.date and series.points.
 * - series.yData and series.processedYData contain clean x values, equivalent to series.date and series.points.
 *
 * @param {Object} chart
 * @param {Object} options
 */
...
Series.prototype = {
	...
	/**
	 * Replace the series date with a new set of date
	 * @param {Object} data
	 * @param {Object} redraw
	 */
	...
	/**
	 * Process the date by cropping away unused date points if the series is longer
	...
	processData: function (force) {
		...
		for (i = processedXData.length - 1; i >= 0; i--) {
			...
			// Unsorted date is not supported by the line tooltip, as well as date grouping and
	...
	/**
	 * Generate the date point after the date has been processed by cropping away
	...
	generatePoints: function () {
		...
		for (i = 0; i < processedDataLength; i++) {
			...
			if (!hasGroupedData) {
				...
				// splat the y date in case of ohlc date array
				points[i] = (new pointClass()).init(series, [processedXData[i]].concat(splat(processedYData[i])));
			}
		}

		// Hide cropped-away points - this only runs when the number of points is above cropThreshold, or when
		// swithching view from non-grouped date to grouped date (#637)
	...
	/**
	 * Translate date points from raw date values to chart specific positioning date
...
var PieSeries = {
	...
	translate: function (positions) {
		...
		// utility for getting the x value from a given y, used for anticollision logic in date labels
	...
	/**
	 * Verify whether the date labels are allowed to draw, or we should run more translation and date
	 * label positioning to keep them inside the plot area. Returns true when date labels are ready
	 * to draw.
	 */
